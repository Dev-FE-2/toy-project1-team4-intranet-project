...
// 공지사항 검색 (search Query 포함)
router.post('/notice/search', (req, res) => {
	const { searchQuery, page = 1, limit = 4 } = req.body;
	const offset = (page - 1) * limit;
	...
	db.get(countQuery, [searchTerm, searchTerm], (err, countResult) => {
		...
		const totalNotices = countResult.count;
		...
		const dataQuery = `
            ...
            FROM notice
            LEFT JOIN images ON notice.image_id = images.id
            WHERE notice.title LIKE ? OR notice.content LIKE ?
...
function saveImageAndNotice(imageData, imageName, noticeData, res) {
	db.serialize(() => {
		...
		db.run(`INSERT INTO images (name, data) VALUES (?, ?)`, [imageName, imageData], function (err) {
			if (err) {
